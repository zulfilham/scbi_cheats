function initialization()
   ...
   informations = {
      root_pointer_names = {[1] = "items", [4] = "neomall"},
      pointer_names = {[11] = "refresh_simcash"},
      value_type = gg.TYPE_DWORD,
      memory_regions = gg.REGION_C_ALLOC | gg.REGION_C_HEAP,
      neomall_refresh_time = 21600000,
      count_offsets = {},
      show_help = true,
   ...
   menus = {
      main = {
         {"UBAH SIMCASH PENYEGARAN ULANG", "BUKA GAMEGUARDIAN", "EKSIT"}, {"🌕   UBAH SIMCASH PENYEGARAN ULANG", "🌕   BUKA GAMEGUARDIAN", "🌕   EKSIT"},
         prompt = "Pilih menu: "
      }
   };
   ...
   if current_informations.x64 then
      ...
      informations.offsets = {refresh_simcash = {0x94}, neomall_refresh_simcash = {0x8, 0x10, 0x18}, neomall_refresh_time = {0x30}};
      root_pointers = {items = {constant_value = -113243136, address_pattern = "1FC"}, neomall = {constant_value = -788267008, address_pattern = "92C"}};
      parent_pointers = {items = {count = 55}, neomall = {count = 1}};
      pointers = {refresh_simcash = {position = 27, count = 4}, neomall = {position = 1, count = -1}};
   else
      informations.address_type = gg.TYPE_DWORD;
      informations.offsets = {refresh_simcash = {0x60}, neomall_refresh_simcash = {0x4, 0x8, 0xc}, neomall_refresh_time = {0x1c}};
      root_pointers = {items = {constant_value = -443547632, address_pattern = "930"}, neomall = {constant_value = -499121980, address_pattern = "1F0"}};
      parent_pointers = {items = {count = 55}, neomall = {count = 1}};
      pointers = {refresh_simcash = {position = 26, count = 4}, neomall = {position = 1, count = -1}};
   end;
   ...
   data = {
      ...
      refresh_simcash = {default_values = {}, current_values = {}}
   };
   ...
   messages = {
      ...
      neomall = {
         search_toast = "Mencari petunjuk akar NeoMall...",
         ambiguous_constant_value = "Peringatan: Petunjuk akar NeoMall ambigu, %d ditemukan",
         root_pointer_not_found = "Eror: Petunjuk akar NeoMall tidak ditemukan",
         parent_pointers_not_found = "Eror: Petunjuk induk NeoMall tidak ditemukan",
         pointers_not_found = {
            "Eror: Petunjuk NeoMall tidak ditemukan"
         }
      },
      neomall_refresh_simcash_not_found = "Eror: Simcash penyegaran ulang NeoMall tidak ditemukan",
      refresh_simcash_altered = "Berhasil mengubah nilai simcash penyegaran ulang",
      refresh_simcash_altered_already = "Anda memang sudah mengubah nilai simcash penyegaran ulang",
...
function change_address(values, offset, flag)
   if flag == nil then flag = gg.TYPE_AUTO; end;
   local results = {};

   for i, value in ipairs(values) do
      results[i] = {address = value.address + offset, flags = flag};
   end;
...
function find_refresh_simcash()
   find_pointers(informations.root_pointer_names[1], informations.pointer_names[11]);

   for i, pointer in ipairs(data[informations.root_pointer_names[1]].pointers[informations.pointer_names[11]]) do
      data.refresh_simcash.current_values[i] = {address = pointer.address + informations.offsets.refresh_simcash[1], flags = informations.value_type};
   end;
   find_neomall_refresh_simcash();
   data.refresh_simcash.current_values = gg.getValues(data.refresh_simcash.current_values);
   data.refresh_simcash.default_values = copy_values(data.refresh_simcash.current_values);
end;

function find_neomall_refresh_simcash()
   find_pointers(informations.root_pointer_names[4]);
   local neomall_refresh_time = change_address(data[informations.root_pointer_names[4]].pointers, informations.offsets.neomall_refresh_time[1], informations.value_type);
   local value_not_found = true;

   for i, pointer in ipairs(neomall_refresh_time) do
      if pointer.value == informations.neomall_refresh_time then
         value_not_found = false;
         local count_refresh_simcash = #data.refresh_simcash.current_values;

         for j, offset in ipairs(informations.offsets.neomall_refresh_simcash) do
            count_refresh_simcash = count_refresh_simcash + 1
            data.refresh_simcash.current_values[count_refresh_simcash] = {address = data[informations.root_pointer_names[4]].pointers[i].address + offset, flags = informations.value_type};
         end;
         break;
      end;
   end;
   ...
   if value_not_found then
      gg.alert(messages.neomall_refresh_simcash_not_found, messages.positive[1]);
      os.exit();
   end;
end;

function main_menu()
   gg.setValues(data.refresh_simcash.default_values);

   if type(cache.data.alter_refresh_simcash) ~= "table" then
      cache.data.alter_refresh_simcash = {};
   end;

   local choices = {};
   local default_choice = nil;

   if math.type(cache.data.alter_refresh_simcash.main_menu) == "integer" then
      if cache.data.alter_refresh_simcash.main_menu >= 1 and cache.data.alter_refresh_simcash.main_menu <= menus.main.count then
         default_choice = cache.data.alter_refresh_simcash.main_menu;
      end;
   end;
   ...
   if choice then
      ...
      if choice < menus.main.count then
         ...
         cache.data.alter_refresh_simcash.main_menu = choice;
         save_cache_data();
      end;
   else
      menus.main.run = false;
      gg.setValues(data.refresh_simcash.current_values);
   end;
   return choice;
end;

function alter_refresh_simcash(value)
   local message, fast;

   if not data.is_cached.alter_refresh_simcash then
      cache_results();
      find_refresh_simcash();
      data.is_cached.alter_refresh_simcash = true;
      load_results();
      message = messages.refresh_simcash_altered;
      fast = false;
   else
      message = messages.refresh_simcash_altered_already;
      fast = true;
   end;

   for i, refresh_simcash in ipairs(data.refresh_simcash.current_values) do
      refresh_simcash.value = value;
   end;
   gg.setValues(data.refresh_simcash.current_values);
...
function openGameGuardianUi()
   ...
   if informations.show_help and data[informations.root_pointer_names[4]].pointers[1] then
